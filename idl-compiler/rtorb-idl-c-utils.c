...
char *
rtorb_cbe_get_typecode_name (IDL_tree tree)
{
	if (!tree)
		return g_strdup ("TC_FIXME");
	else
		return g_strconcat ("TC_", rtorb_cbe_get_typespec_str (tree), NULL);
}

gboolean
rtorb_cbe_type_is_builtin(IDL_tree tree)
{ 
  return FALSE;
...
**/
char *
rtorb_cbe_get_typespec_str2(IDL_tree tree, OIDL_TypeNameType tn)
{
  char *retval = NULL;
  GString *tmpstr = NULL;
  ...
  switch(IDL_NODE_TYPE(tree)) {
  case IDLN_MEMBER:
    return rtorb_cbe_get_typespec_str2(IDL_MEMBER(tree).type_spec, tn);
    break;
  case IDLN_TYPE_ANY:
  ...
  case IDLN_TYPE_BOOLEAN:
    ...
    break;
  case IDLN_TYPE_STRUCT:
    return rtorb_cbe_get_typespec_str2(IDL_TYPE_STRUCT(tree).ident, tn);
  case IDLN_EXCEPT_DCL:
    return rtorb_cbe_get_typespec_str2(IDL_EXCEPT_DCL(tree).ident, tn);
  case IDLN_TYPE_ARRAY:
    return rtorb_cbe_get_typespec_str2(IDL_TYPE_ARRAY(tree).ident, tn);
  case IDLN_TYPE_UNION:
    return rtorb_cbe_get_typespec_str2(IDL_TYPE_UNION(tree).ident, tn);
  case IDLN_TYPE_ENUM:
    return rtorb_cbe_get_typespec_str2(IDL_TYPE_ENUM(tree).ident, tn);
  case IDLN_IDENT:
  ...
  case IDLN_PARAM_DCL:
    return rtorb_cbe_get_typespec_str2(IDL_PARAM_DCL(tree).param_type_spec, tn);
  case IDLN_TYPE_SEQUENCE:
    {
	IDL_tree subtype = IDL_TYPE_SEQUENCE(tree).simple_type_spec;
        char *ctmp, *base;
        ctmp = rtorb_cbe_get_typespec_str2(subtype, tn);
	/* We should have built-in alias to make this next line not needed */
	base = rtorb_cbe_type_is_builtin(subtype)
  ...
  case IDLN_FORWARD_DCL:
  case IDLN_INTERFACE:
    return rtorb_cbe_get_typespec_str2(IDL_INTERFACE(tree).ident, tn);
  case IDLN_TYPE_OBJECT:
    if (tn == OIDL_TYPENAME_C) {
      retval = "CORBA_Object";
    } else {
...
char *
rtorb_cbe_get_typespec_str(IDL_tree tree)
{
     return rtorb_cbe_get_typespec_str2(tree, OIDL_TYPENAME_C);
}
void
rtorb_cbe_write_typespec(FILE *of, IDL_tree tree)
{
    char *name = rtorb_cbe_get_typespec_str(tree);
    fprintf( of, "%s", name);
    g_free(name);
}
...
char *
rtorb_cbe_write_param_typespec_str3(IDL_tree ts, IDL_ParamRole role, OIDL_TypeNameType otype)
{
  int      i, n;
  gboolean isSlice;
  char    *name;
  ...
  name = rtorb_cbe_get_typespec_str2 (ts, otype);
  typedef_spec = orbit_cbe_get_typespec (ts);
  typedef_name = rtorb_cbe_get_typespec_str(typedef_spec);
...
char *
rtorb_cbe_write_param_typespec_str(IDL_tree ts, IDL_ParamRole role)
{
     return rtorb_cbe_write_param_typespec_str3(ts, role, OIDL_TYPENAME_C);
}


static void
rtorb_cbe_write_param_typespec_raw4 (FILE *of, IDL_tree ts, IDL_ParamRole role, OIDL_TypeNameType tn)
{
    char *str;
    str = rtorb_cbe_write_param_typespec_str3 (ts, role, tn);
    fprintf (of, "%s", str);
    g_free (str);
}

void
rtorb_cbe_write_param_typespec3(FILE *of, IDL_tree tree, OIDL_TypeNameType tn) 
{
    ...
    rtorb_cbe_write_param_typespec_raw4(of, ts, role, tn);
}

void
rtorb_cbe_write_param_typespec(FILE *of, IDL_tree tree) 
{
     rtorb_cbe_write_param_typespec3(of, tree, OIDL_TYPENAME_C);
}

char *
rtorb_cbe_get_param_typespec_str3(IDL_tree ts, IDL_ParamRole role, OIDL_TypeNameType otype)
{
  int      i, n;
  gboolean isSlice;
  char    *name;
...
char *
rtorb_cbe_get_param_typespec_str2(IDL_tree tree, OIDL_TypeNameType tn) 
{
    IDL_tree		ts = NULL /* Quiet gcc */;
    IDL_ParamRole	role = 0 /* Quiet gcc */;
    ...
    return rtorb_cbe_get_param_typespec_str3 (ts, role, tn);
}

char *
rtorb_cbe_get_param_typespec_str(IDL_tree tree) 
{
     return rtorb_cbe_get_param_typespec_str2(tree, OIDL_TYPENAME_C);
}

int
rtorb_cbe_get_param_typespec(IDL_tree tree)
{
    IDL_tree		ts = NULL /* Quiet gcc */;
    ...
    return (int)IDL_NODE_TYPE(orbit_cbe_get_typespec(ts));
}

void
rtorb_cbe_op_write_proto (FILE       *of,
			  IDL_tree    op,
			  ...
			  gboolean    for_epv)
{
	...
	rtorb_cbe_write_param_typespec (of, op);
	...
	for (sub = IDL_OP_DCL (op).parameter_dcls; sub; sub = IDL_LIST (sub).next) {
		...
		rtorb_cbe_write_param_typespec (of, parm);
...
static char *
rtorb_cbe_get_const(IDL_tree tree)
{
  ...
  case IDLN_BINOP:
    ...
    ctmp = rtorb_cbe_get_const(IDL_BINOP(tree).left);
    ...
    ctmp = rtorb_cbe_get_const(IDL_BINOP(tree).right);
  ...
  case IDLN_UNARYOP:
    ...
    ctmp = rtorb_cbe_get_const(IDL_UNARYOP(tree).operand);
...
void
rtorb_cbe_write_const(FILE *of, IDL_tree tree)
{
  char *ctmp;

  ctmp = rtorb_cbe_get_const(tree);
  fprintf(of, "%s", ctmp);
  g_free(ctmp);
}
...
void
rtorb_cbe_id_define_hack(FILE *fh, const char *def_prefix, const char *def_name, const char *def_value)
{
  int i, n;
  n = strlen(def_value);
  for(i = 0; i < n; i++)
...
void
rtorb_cbe_id_cond_hack(FILE *fh, const char *def_prefix, const char *def_name, const char *def_value)
{
  int i, n;
  n = strlen(def_value);
  if(n <= 0)
...
static const char *
rtorb_cbe_flatten_ref (IDL_ParamRole role, IDL_tree typespec)
{
	gboolean is_fixed;
...
void
rtorb_cbe_flatten_args (IDL_tree tree, FILE *of, const char *name)
{
	int i = 0;
	IDL_tree l;
...
char *
rtorb_cbe_unflatten_ref (IDL_ParamRole role, IDL_tree typespec)
{
	gboolean is_fixed;
	char    *typestr;
	char    *retval;
	...
	typestr = rtorb_cbe_write_param_typespec_str (typespec, role);
...
void
rtorb_cbe_unflatten_args (IDL_tree tree, FILE *of, const char *name)
{
	IDL_tree l;
	int      i = 0;
	...
	for (l = IDL_OP_DCL(tree).parameter_dcls; l;
		...
		unflatten = rtorb_cbe_unflatten_ref (r, tspec);
