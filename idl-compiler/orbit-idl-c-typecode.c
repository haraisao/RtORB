...
/*
  Main function to output typecode definitions
...
orbit_output_tcstruct_kind (FILE *fh, IDL_tree node, int array_gen_ctr)
{
	...
	case IDLN_INTERFACE:
	case IDLN_FORWARD_DCL:
		fprintf (fh, "tk_objref");
		break;
	case IDLN_EXCEPT_DCL:
		fprintf (fh, "tk_except");
		break;
	case IDLN_TYPE_INTEGER:
		fprintf (fh, "tk_");
	...
	case IDLN_TYPE_FLOAT:
		...
		break;
	case IDLN_TYPE_BOOLEAN:
		fprintf (fh, "tk_boolean");
		break;
	case IDLN_TYPE_OCTET:
		fprintf (fh, "tk_octet");
		break;
	case IDLN_TYPE_STRING:
		fprintf (fh, "tk_string");
		break;
	case IDLN_TYPE_WIDE_STRING:
		fprintf (fh, "tk_wstring");
		break;
	case IDLN_TYPE_CHAR:
		fprintf (fh, "tk_char");
		break;
...
orbit_output_tcstruct_digits_scale (FILE *fh, IDL_tree node)
{
	if (IDL_NODE_TYPE (node) == IDLN_TYPE_FIXED) {
		fprintf (fh, "%" IDL_LL "d, %" IDL_LL "d" ,
			 (long long)IDL_INTEGER (IDL_TYPE_FIXED (node).positive_int_const).value,
			 (long long)IDL_INTEGER (IDL_TYPE_FIXED (node).integer_lit).value);
	}
	else
		fprintf (fh, "0, 0");
}
...
orbit_find_c_align (GSList *max, IDL_tree node)
{
	...
	case IDLN_TYPE_INTEGER:
		switch (IDL_TYPE_INTEGER (node).f_type) {
		case IDL_INTEGER_TYPE_SHORT:
			orbit_add_align (&max, "Align_short");
			break;
		case IDL_INTEGER_TYPE_LONG:
			orbit_add_align (&max, "Align_long");
			break;
		case IDL_INTEGER_TYPE_LONGLONG:
			orbit_add_align (&max, "Align_longlong");
			break;
		}
		break;
	case IDLN_TYPE_FLOAT:
		switch (IDL_TYPE_FLOAT (node).f_type) {
		case IDL_FLOAT_TYPE_FLOAT:
			orbit_add_align (&max, "Align_float");
			break;
		case IDL_FLOAT_TYPE_DOUBLE:
			orbit_add_align (&max, "Align_double");
			break;
		case IDL_FLOAT_TYPE_LONGDOUBLE:
			orbit_add_align (&max, "Align_longdouble");
			break;
		}
		break;
	case IDLN_TYPE_ENUM:
	...
	case IDLN_TYPE_BOOLEAN:
	case IDLN_TYPE_OCTET:
		orbit_add_align (&max, "Align_char");
		break;
	case IDLN_TYPE_WIDE_CHAR:
		orbit_add_align (&max, "Align_short");
		break;
	case IDLN_TYPE_UNION: {
		...
		orbit_add_align (&max, "Align_union");
	...
	case IDLN_FORWARD_DCL:
	case IDLN_INTERFACE:
		orbit_add_align (&max, "Align_pointer");
		break;
	case IDLN_TYPE_ARRAY: {
	...
	case IDLN_TYPE_SEQUENCE:
		orbit_add_align (&max, "Align_sequence");
		orbit_add_align (&max, "Align_long");
		orbit_add_align (&max, "Align_pointer");
		break;
	case IDLN_TYPE_ANY:
...
*/
static int
orbit_find_c_size (IDL_tree node)
{
  int size=0;
  node = orbit_cbe_get_typespec (node);	
  ...
  case IDLN_TYPE_INTEGER:
	...
	case IDLN_TYPE_UNION: {
		...
		for (; l; l = IDL_LIST (l).next) {
			IDL_tree subtype = IDL_MEMBER (IDL_CASE_STMT (
				IDL_LIST (l).data).element_spec).type_spec;
			size += orbit_find_c_size (subtype);
		}
		break;
	}
	case IDLN_EXCEPT_DCL: /* drop through */
	...
	case IDLN_TYPE_SEQUENCE:
		size = 4; /* long maximum */
		size += 4; /* long length */
		size += 4; /* pointer buffer */
		size += 4; /* boolean release <= unsigned long */
		break;
	case IDLN_TYPE_ANY:
		size = 4; /* pointer of TypeCode */
		size += 4; /* pointer of any_val */
		size += 4; /* boolean release <= unsigned long */
		break;
	default:
		g_error ("Can't find size %s\n", 
			 IDL_tree_type_names[IDL_NODE_TYPE (node)]);
		break;
	}
