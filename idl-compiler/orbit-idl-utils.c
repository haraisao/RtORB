...
orbit_idl_print_node(IDL_tree node, int indent_level)
{
  ...
  case IDLN_INTEGER:
    g_print("INTEGER: %" IDL_LL "d\n", (long long)IDL_INTEGER(node).value);
    break;
...
oidl_param_info(IDL_tree in_param, IDL_ParamRole role, OIDL_TypeNameType otype, gboolean *isSlice)
{
	...
	*isSlice = FALSE;
...
#if RETURN_VALUE
	case IDLN_TYPE_SEQUENCE:
		if (role == DATA_RETURN && !OIDL_TYPENAME_IS_C(otype)) {
			retval++;
		}
		break;
	...
	case IDLN_TYPE_STRUCT:
		if (role == DATA_RETURN) {
			/* data type when in C, pointer when in C++ */
			...
			if (!orbit_cbe_type_is_fixed_length(param)) {
				retval++;
			}
		}
		break;
