...
cbe_skel_interface_print_relayers(const CBESkelInterfaceTraverseInfo *iti)
{
  ...
  for(i=0; i < iti->curlevel ; i++){ fprintf(iti->ci->fh, "    "); }
  ...
  while(curnode) {
    ...
    while(curnode && ((CBESkelOpInfo *)curnode->data)->opname[iti->curlevel] == curchar) {
    ...
    if(g_slist_length(subiti.oplist) > 1) {
      ...
      for(i=0; i < iti->curlevel ; i++){ fprintf(iti->ci->fh, "    "); }
      fprintf(iti->ci->fh, "      *impl = (void *)servant->vepv->%s_epv->%s;\n",
        opi->iface_id, opi->opname);
      for(i=0; i < iti->curlevel ; i++){ fprintf(iti->ci->fh, "    "); }
      fprintf(iti->ci->fh, "      *m_data = &%s__imp.methods [%d];\n",
        opi->iface_id, opi->idx);
      for(i=0; i < iti->curlevel ; i++){ fprintf(iti->ci->fh, "    "); }
      fprintf(iti->ci->fh, "      return (void *)_RtORB_impl_%s_%s;\n",
        opi->iface_id, opi->opname);
    }
    for(i=0; i < iti->curlevel ; i++){ fprintf(iti->ci->fh, "    "); }
...
cbe_skel_interface_print_relayer(IDL_tree tree, OIDL_Run_Info *rinfo, OIDL_C_Info *ci)
{
  ...
  */
  fprintf(ci->fh, "static void * get_skel_small_%s(POA_%s *servant,\nconst char *opname,"
...
cbe_skel_do_interface(IDL_tree tree, OIDL_Run_Info *rinfo, OIDL_C_Info *ci)
{
  ...
  fprintf(ci->fh,
    "void POA_%s__init(PortableServer_Servant servant, CORBA_Environment *env)\n",
    id);
  fprintf(ci->fh, "{\n");
...
#else
  fprintf (ci->fh, "  RtORB_POA_Object__create (&class_info,\n");
  fprintf (ci->fh, "  servant, NULL,\n");
  fprintf (ci->fh, "  NULL);\n");
#endif
  fprintf(ci->fh, "}\n\n");
  ...
  if(IDL_INTERFACE(tree).inheritance_spec)
    {
      for(i = IDL_list_length(IDL_INTERFACE(tree).inheritance_spec) - 1; i >= 0; i--) {
        curitem = IDL_list_nth(IDL_INTERFACE(tree).inheritance_spec, i);
        id2 = IDL_ns_ident_to_qstring(IDL_IDENT_TO_NS(IDL_LIST(curitem).data), "_", 0);
